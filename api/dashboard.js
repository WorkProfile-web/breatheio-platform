...
module.exports = async (req, res) => {
  ...
  if (allDevices.length === 0) {
    ...
    const sorted = [...allDevices].sort((a, b) => {
      const aOn = (now - a.lastSeen) < 120000, bOn = (now - b.lastSeen) < 120000;
      if (aOn && !bOn) return -1;
      if (!aOn && bOn) return 1;
      return (a.name || '').localeCompare(b.name || '');
    });
    for (const d of sorted) {
      const on = (now - d.lastSeen) < 120000;
  ...
  const online = allDevices.filter(d => (now - d.lastSeen) < 120000).length;
...
var dv=d.devices[i];
var now=Date.now();
var isOn=(now-dv.lastSeen)<120000;
if(isOn)on++;else off++;
